...
function abrirPagina(tipo) {
    ...
    container.innerHTML = `
        ...
        <button id="botaoAlerta" onclick="abrirPagina('alerta')">Configurar Alerta</button>
        <button id="botaoScript" onclick="abrirPagina('script')">Configurar Script</button>
        <button id="botaoSla" onclick="abrirPagina('sla')">Configurar SLA</button>
    `;

    const novoTitulo = document.getElementById("tituloPagina");

    if (tipo === "alerta") {
        novoTitulo.innerHTML = "Configurar Parâmetros de Alerta";
        carregarPaginaAlerta();
    }

    if (tipo === "script") {
        novoTitulo.innerHTML = "Configurar Parâmetros de Script";
        carregarPaginaScript();
    }

    if (tipo === "sla") {
        novoTitulo.innerHTML = "Configurar Parâmetros de SLA";
        carregarPaginaSla();
    }
}*/

/*
function carregarPaginaAlerta() {
    window.location = "./configAlerta.html"
    
    document.getElementById('configuracaoContainer').innerHTML += `
        <div>conteúdo da página de alerta...</div>
    `;
    
}

function carregarPaginaScript() {
    document.getElementById('configuracaoContainer').innerHTML += `
        <div>conteúdo da página de script...</div>
    `;
}

function carregarPaginaSla() {
    
    
    document.getElementById('configuracaoContainer').innerHTML += `
        <div>conteúdo da página de SLA...</div>
    `;
...
function alterarPaginaSla(){
    if (opcao == 0){
        opcao = 1
        document.getElementById('tituloPagina').innerHTML = 'Configurar Parâmetros de Sla'
        document.getElementById('botaoTrocaSla').innerHTML = 'Configurar Alerta'
    }
    else if (opcao == 1){
        opcao = 0
        document.getElementById('tituloPagina').innerHTML = 'Configurar Parâmetros de Alerta'
        document.getElementById('botaoTrocaSla').innerHTML = 'Configurar Sla'
    }
